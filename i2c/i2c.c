 ...
 * Note [7]
 *
 * Read "len" bytes from HMC5883 starting at "eeaddr" into "buf".
 *
 ...
 * for 16 bit address devices (larger HMC5883 devices)
 *
 ...
 */
int
read_bytes(uint16_t eeaddr, int len, uint8_t *buf)
{
  uint8_t sla, twcr, n = 0;
  int rv = 0;

#ifndef WORD_ADDRESS_16BIT
  /* patch high bits of HMC5883 address into SLA */
  sla = TWI_SLA_HMC5883 | (((eeaddr >> 8) & 0x07) << 1);
#else
...
int
write_bytes(uint16_t eeaddr, int len, uint8_t *buf)
{
  uint8_t sla, n = 0;
  int rv = 0;
